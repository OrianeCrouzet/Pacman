...
public class SpecialGhost extends Ghosts{
    ...
    private boolean tryAStarMove() {
        Node start = new Node(x / Cell.SIZE, y / Cell.SIZE);
        Node target = new Node(
            lab.getPersonnages().getPacman().x / Cell.SIZE,
            lab.getPersonnages().getPacman().y / Cell.SIZE
        );
    ...
    private void followPath(List<Node> path) {
        ...
        int dx = nextNode.x * Cell.SIZE + Cell.SIZE/2 - (x + GHOST_WIDTH/2);
        int dy = nextNode.y * Cell.SIZE + Cell.SIZE/2 - (y + GHOST_HEIGHT/2);
